...
export const createBooking = async (req, res) => {
  const { saathiId, serviceType, date, time, duration, notes, location, quotedPrice } = req.body;
  ...
  await Payment.create({
    ...
    amount: quotedPrice || calculateAmount(duration),
    status: "paid",
    method: "Mock UPI"
  });
...
export const updateBookingStatus = async (req, res) => {
  ...
  if (!isDbConnected()) {
    await initializeMockData();
    const booking = await updateMockBooking(req.params.id, {
      status,
      liveStatus,
      quotedPrice,
    ...
    return res.json(await populateMockBooking(booking));
  }
...
export const getOpenRequests = async (req, res) => {
  ...
  const bookings = await Booking.find({
    ...
    .populate({ path: "elderId", select: "name phone address profilePhoto" })
    .sort({ createdAt: -1 })
    .limit(5);
...
export const submitVisitReport = async (req, res) => {
  ...
  if (!isDbConnected()) {
    ...
    const report = await upsertMockVisitReport(booking._id, {
      tasksCompleted,
      elderMood,
      concerns
    });
...
export const rateBooking = async (req, res) => {
  ...
  if (!isDbConnected()) {
    await initializeMockData();
    const booking = await rateMockBooking(req.params.id, rating);
    if (!booking) {
      return res.status(404).json({ message: "Booking not found" });
    }
...
export const triggerSos = async (req, res) => {
  ...
  console.log(`SOS alert triggered by ${req.user.name} at ${location}`);
